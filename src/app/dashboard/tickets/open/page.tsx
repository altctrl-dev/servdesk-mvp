...
const PAGE_STATUS = "OPEN" as const;

interface OpenTicketsPageProps {
...
async function OpenTicketsContent({
  searchParams,
}: {
  searchParams: {
    priority?: string;
  ...
  const ticketData = await getOpenTickets(db, session.user.id, canViewAll, {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Open Tickets</h1>
        <p className="text-muted-foreground">
          Tickets that are currently open and awaiting resolution
        </p>
      </div>
...
export default async function OpenTicketsPage({
  searchParams,
}: OpenTicketsPageProps) {
  const resolvedSearchParams = await searchParams;

  return (
    <Suspense fallback={<OpenTicketsSkeleton />}>
      <OpenTicketsContent searchParams={resolvedSearchParams} />
    </Suspense>
  );
}
