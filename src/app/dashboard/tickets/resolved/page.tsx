...
const PAGE_STATUS = "RESOLVED" as const;

interface ResolvedTicketsPageProps {
...
async function ResolvedTicketsContent({
  searchParams,
}: {
  searchParams: {
    priority?: string;
  ...
  const ticketData = await getResolvedTickets(db, session.user.id, canViewAll, {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Resolved Tickets</h1>
        <p className="text-muted-foreground">
          Tickets that have been resolved and are awaiting closure
        </p>
      </div>
...
function ResolvedTicketsSkeleton() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Resolved Tickets</h1>
...
export default async function ResolvedTicketsPage({
  searchParams,
}: ResolvedTicketsPageProps) {
  const resolvedSearchParams = await searchParams;

  return (
    <Suspense fallback={<ResolvedTicketsSkeleton />}>
      <ResolvedTicketsContent searchParams={resolvedSearchParams} />
    </Suspense>
  );
}
