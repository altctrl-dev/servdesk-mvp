 ...
 * Closed Tickets Page
 *
 * Displays all tickets with "CLOSED" status.
...
async function ClosedTicketsContent({
  searchParams,
}: {
  searchParams: {
    priority?: string;
  ...
  const ticketData = await getClosedTickets(db, session.user.id, canViewAll, {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Closed Tickets</h1>
        <p className="text-muted-foreground">
          Tickets that have been closed and completed
        </p>
      </div>
...
export default async function ClosedTicketsPage({
  searchParams,
}: ClosedTicketsPageProps) {
  const resolvedSearchParams = await searchParams;

  return (
    <Suspense fallback={<ClosedTicketsSkeleton />}>
      <ClosedTicketsContent searchParams={resolvedSearchParams} />
    </Suspense>
  );
}
